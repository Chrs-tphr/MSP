...
|         : 07.17.2016 - 001: Updated populateDecalNumbers() to issue new decal number on transfer
|         : 07.17.2016 - 002: Created setEqListStatus()
|         : 07.17.2016 - 003: Created updateContinuousContract()
|         : 07.17.2016 - 004: Updated updateCert() to set TD Inactive status for Full Temp Discontinuance and null status on Full Perm Discontinuance
|         : 07.17.2016 - 005: Updated updateCert() to remove PD Contracts and update all others
|         : 07.21.2016 - 006: Updated createCertOfAuth() to check for existing Certificate of Authority and update existing record if found.
|         : 07.21.2016 - 007: Created removeAppTransferRelationship() to replace 1.0 StandardChoice Script
|         : 07.27.2016 - Reverted update 006
|
|
| Defect Updates
|         : 10.14.2016 - 008: Updated updateCertEqListFromRenewal() to work on "Completed" renewals
|
/------------------------------------------------------------------------------------------------------*/
...
//TCH@Accela 2016-09-26 Added test for feeAmt before comparing to the exstngRnwlFeeAmt
...
function createCertOfAuth(){//006 updates removed
...
function updateCert(updateType){
	...
	switch ("" + updateType) {
		...
		case "DISCONTINUANCE":
			...
			cLic = getRefLicenseProf(existingCarrierNum);
			updateContinuousContract(capId, pId);
			break;
		case "TEMPDISCON":
			opType = AInfo["Operation Type"];
			autoTrans = AInfo["Auto Transport"];
			...
			cLic = getRefLicenseProf(existingCarrierNum);
			if (matches(AInfo["Type"], "Full") && matches(AInfo["Duration"], "Temporary")){
				setEqListStatus("TD Inactive", pId, pId);
			}
			updateContinuousContract(capId, pId);
			break;
		case "PERMDISCON":
			...
			editRefLicProfAttribute(existingCarrierNum, "INTRASTATE AUTHORITY STATUS DA", dateAdd(effDate, 0));
			cLic = getRefLicenseProf(existingCarrierNum);
			setEqListStatus("Discontinued", pId, pId);
			updateContinuousContract(capId, pId);
			break;
		case "REINSTATE":
			...
			editRefLicProfAttribute(existingCarrierNum, "INTRASTATE AUTHORITY STATUS DA", dateAdd(effDate, 0));
			cLic = getRefLicenseProf(existingCarrierNum);
			setEqListStatus("Active", pId, pId);
			updateContinuousContract(capId, pId);
			break;
		case "EQUIPLIST":
...
function updateContinuousContract(sRecord, tRecord){
	ccTable = loadASITable("CONTINUOUS CONTRACT", sRecord);
	newTable = new Array();
	for (var eachRow in ccTable) {
		thisRow = ccTable[eachRow];
		newRow = new Array();
		var ccStatus = "" + thisRow["Status"];

		if (!matches(ccStatus, "Permanently Discontinued")){
			newRow["Name"] = new asiTableValObj("Name", thisRow["Name"].fieldValue, "N");
			newRow["Description"] = new asiTableValObj("Description", thisRow["Description"].fieldValue, "N");
			if(matches(""+ccStatus, "Reinstate")){
				newRow["Status"] = new asiTableValObj("Status", "Active", "N");
			}else{
				newRow["Status"] = new asiTableValObj("Status", thisRow["Status"].fieldValue, "N");
			}
			newTable.push(newRow);
	...
	removeASITable("CONTINUOUS CONTRACT", tRecord);
	addASITable("CONTINUOUS CONTRACT", newTable, tRecord);
}
...
function populateDecalNumbers(){
	...
	for (var eachRow in eqListTable) {
		...
		if (matches(vAction, "Add Vehicle", "Replacement Decal", "Renew", "Transfer") && vStatus == "Active") {
			sessID = getSessionID();
...
function setEqListStatus(vStatus, sRecord, tRecord){
	eqListTable = loadASITable("EQUIPMENT LIST", sRecord);
	newTable = new Array();
	for (var eachRow in eqListTable) {
		thisRow = eqListTable[eachRow];
		newRow = new Array();
		...
		newRow["Leased Vehicle Owner"] = new asiTableValObj("Leased Vehicle Owner", thisRow["Leased Vehicle Owner"].fieldValue, "N");
		newRow["Vehicle Action"] = new asiTableValObj("Vehicle Action", thisRow["Vehicle Action"].fieldValue, "N");
		newRow["Status"] = new asiTableValObj("Status", vStatus, "N");
		newRow["MPSC Decal #"] = new asiTableValObj("MPSC Decal #", thisRow["MPSC Decal #"].fieldValue, "Y");
		newRow["Equipment Use"] = new asiTableValObj("Equipment Use", thisRow["Equipment Use"].fieldValue, "N");
		newRow["Plate Fee"] = new asiTableValObj("Plate Fee", thisRow["Plate Fee"].fieldValue, "N");
		newTable.push(newRow);
	}
	removeASITable("EQUIPMENT LIST", tRecord);
	addASITable("EQUIPMENT LIST", newTable, tRecord);
}
...
function createRefLicProfFromLicProf(recordId) {
	//
	// Get the lic prof from the app
	//
	capLicenseResult = aa.licenseScript.getLicenseProf(recordId);
	if (capLicenseResult.getSuccess()) {
		capLicenseArr = capLicenseResult.getOutput();
	} else {
		logDebug("**ERROR: getting lic prof: " + capLicenseResult.getErrorMessage());
